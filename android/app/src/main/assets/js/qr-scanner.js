 ...
 * Wraps the external html5-qrcode library to handle camera initialization, overlays, and detection hooks.
 */
const QRScanner = (() => {
   ...
   * @param {function(string)} options.onScan - Callback with decoded location ID
  ...
  async function start(containerId, options = {}) {
    ...
    try {
      ...
      const config = {
        ...
        formatsToSupport: [Html5QrcodeSupportedFormats.QR_CODE] // Restrict matching to QR Codes only
      };
   ...
   * This is invoked continuously on every frame when no QR code is in view.
   * We ignore it to keep console outputs clear.
   */
