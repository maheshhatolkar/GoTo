 ...
 */
export function bearing(idA, idB) {
  const a = locations[idA];
  const b = locations[idB];
  if (!a || !b) return 0;
  const dx = b.x - a.x;
 ...
 */
export function getTurnDirection(prevId, currentId, nextId) {
 ...
 */
export function findShortestPath(sourceId, destId) {
...
// Default export holding all exports for convenience
const LocationGraph = {
  locations,
  getLocation,
  isValidLocation,
